...
- **[fork-tools/verification/](fork-tools/verification/)** — three checks a unit test cannot express, because they need separate OS processes, a genuine race, or data this machine's accounts never produce. Two run in CI on all three OSes. The leader-election one caught a real bug (3 leaders out of 8) that no single-process test would notice, so **don't delete it during a refactor of `leader.ts`**. Its README states what each one establishes, with dates and numbers — re-run and update those rather than trusting them.
